...
const HomeScreen = ({navigation}) => {
  ...
  function renderRestaurantList() {
    const renderItem = ({ item }) => (
      <TouchableOpacity style={{ marginBottom: SIZES.padding * 2 }} onPress={()=>navigation.navigate('Restaurant',{
        ...
        <View style={{ marginBottom: SIZES.padding }}>
          <Image source={item.photo} resizeMode={"cover"} style={{ width: "100%", height: 200 }}
                 borderRadius={SIZES.radius} />
    ...
    return (
      <FlatList data={restaurants} renderItem={renderItem} keyExtractor={item => `${item.id}`} contentContainerStyle={{
